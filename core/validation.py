...
def validate_bottle_sample_range(event: core_models.Event, bottle_id: int) -> \
    ...
    if event.instrument.type == core_models.InstrumentType.ctd and \
        ...
        err = core_models.EventError(event=event, message=message, type=core_models.ErrorType.bottle)
        errors.append(err)
...
def validate_samples_for_biochem(mission: core_models.Mission,
                                 sample_types: list[core_models.MissionSampleType] = None) -> list[core_models.MissionError]:
    errors = []
    ...
    for sample_type in sample_types:
        if not sample_type.datatype:
            ...
            err = core_models.MissionError(mission=mission, message=message, type=core_models.ErrorType.biochem)
            errors.append(err)
