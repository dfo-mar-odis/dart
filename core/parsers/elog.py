...
def validate_message_object(buffer: dict, elog_configuration: core_models.ElogConfig) -> []:
    errors = []
    required_fields = elog_configuration.mappings.filter(required=True)
    for field in required_fields:
        if field.mapped_to not in buffer:
            errors.append(ValueError({'message': _('Message object missing key'),
                                      'key': field.field, 'expected': field.mapped_to}))
...
def parse(stream: io.StringIO, elog_configuration: core_models.ElogConfig) -> dict:
    ...
    for mid in paragraph:
        ...
        mid_errors = validate_message_object(buffer, elog_configuration)
        if mid_errors:
...
def process_stations(station_queue: []):
    ...
    for station in station_queue:
        if not existing_stations.filter(name__iexact=station).exists():
...
def process_instruments(instrument_queue: []):
    # create any stations on the stations queue that don't exist in the DB
    instruments = []
    existing_instruments = core_models.Instrument.objects.all()
    for instrument in instrument_queue:
        if not existing_instruments.filter(name__iexact=instrument).exists():
...
def get_field(elog_configuration: core_models.ElogConfig, mid: str, field: str, buffer: []) -> str:
    mapped_field = elog_configuration.get_mapping(field)

    if mapped_field not in buffer:
        raise ValueError({'message': _('Message object missing key'),
                          'mid': mid, 'key': field, 'expected': mapped_field})

    return mapped_field


def process_events(mid_dictionary_buffer: {}, mission: core_models.Mission) -> []:
    errors = []
    ...
    mids = mid_dictionary_buffer.keys()
    for mid in mids:
        try:
            buffer = mid_dictionary_buffer[mid]

            event_field = get_field(elog_configuration, mid, 'event', buffer)
            station_field = get_field(elog_configuration, mid, 'station', buffer)
            instrument_field = get_field(elog_configuration, mid, 'instrument', buffer)
            sample_id_field = get_field(elog_configuration, mid, 'start_sample_id', buffer)
            end_sample_id_field = get_field(elog_configuration, mid, 'end_sample_id', buffer)
            ...
            # if the event doesn't already exist, create it. Otherwise update the existing event with new data if required
            ...
            logger.exception(ex)
            errors.append(
                (_("Error processing events for $@MID@$ :") + mid + ", " + _("see error.log for details"), ex)
            )
...
def process_attachments_actions(mid_dictionary_buffer: {}, mission: core_models.Mission, file_name: str) -> []:
    errors = []
    ...
    for mid in mids:
        ...
        try:
            event_field = get_field(elog_configuration, mid, 'event', buffer)
            attached_field = get_field(elog_configuration, mid, 'attached', buffer)
            time_position_field = get_field(elog_configuration, mid, 'time_position', buffer)
            comment_field = get_field(elog_configuration, mid, 'comment', buffer)
            action_field = get_field(elog_configuration, mid, 'action', buffer)
            data_collector_field = get_field(elog_configuration, mid, 'data_collector', buffer)
            ...
            if re.search(".*\|.*\|.*\|.*", buffer[time_position_field]) is None:
                raise KeyError({'message': _("Badly formatted or missing Time|Position"), 'key': 'time_position',
            ...
            message = _("Missing or improperly set attribute for $@MID@$ :") + mid + ", " + \
            ...
            logger.exception(ex)
            errors.append(
                (_("Error processing attachments for $@MID@$ :") + mid + ", " + _("see error.log for details"), ex)
            )
...
# anything with it.
def process_variables(mid_dictionary_buffer: {}, mission: core_models.Mission):
    errors = []
    ...
    for mid in mids:
        buffer = mid_dictionary_buffer[mid]
        try:
            lead_scientists_field = get_field(elog_configuration, mid, 'lead_scientist', buffer)
            protocol_field = get_field(elog_configuration, mid, 'protocol', buffer)
            cruise_field = get_field(elog_configuration, mid, 'cruise', buffer)
            platform_field = get_field(elog_configuration, mid, 'platform', buffer)
            ...
            message = _("Error processing variables for $@MID@$ :") + mid + ", " + _("see error.log for details")
            logger.exception(ex)
            errors.append(
                (message, ex)
            )
